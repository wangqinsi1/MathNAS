...
def start(imagenet_save_path):
    ...
    model.cuda()
    logger.info(str(model))
    dataset_train, dataset_val, data_loader_train, data_loader_val, mixup_fn = build_loader(config,imagenet_save_path)
...
def generate_model(subconfig, config, model, data_loader_train, data_loader_val):
    criterion = nn.CrossEntropyLoss()
    acc1, acc5, loss, flops, params = attentive_nas_eval.validate(
        subconfig,
        data_loader_train,
        data_loader_val,
        model,
